...
-- ============================================
-- Resincronización de secuencias (IDENTITY)
-- ============================================
-- Las inserciones anteriores usan IDs explícitos (GENERATED BY DEFAULT AS
-- IDENTITY los permite), pero eso no avanza la secuencia interna. Sin este
-- paso, el próximo INSERT sin ID explícito (hecho por Hibernate) intenta
-- reusar un ID ya ocupado y falla con "duplicate key value violates unique
-- constraint ..._pkey". Se recalcula tras cada carga de este script.
SELECT setval(pg_get_serial_sequence('roles', 'role_id'), COALESCE((SELECT MAX(role_id) FROM roles), 1));
SELECT setval(pg_get_serial_sequence('order_statuses', 'order_status_id'), COALESCE((SELECT MAX(order_status_id) FROM order_statuses), 1));
SELECT setval(pg_get_serial_sequence('payment_methods', 'payment_method_id'), COALESCE((SELECT MAX(payment_method_id) FROM payment_methods), 1));
SELECT setval(pg_get_serial_sequence('payment_statuses', 'payment_status_id'), COALESCE((SELECT MAX(payment_status_id) FROM payment_statuses), 1));
